...
if($path==='/api/passkeys/login/options' && $method==='POST'){
 $retry=rate_limit_consume('passkey-login-ip','',40,900);if($retry>0)json_response(['ok'=>false,'error'=>'Слишком много попыток. Повторите позже.'],429);
 $server=passkey_server();$args=$server->getGetArgs([],300,true,true,true,true,true,'required');passkey_store_challenge('login',0,$server->getChallenge());json_response(['ok'=>true,'publicKey'=>$args->publicKey]);
}
if($path==='/api/passkeys/login' && $method==='POST'){
 try{
  $response=is_array($data['response']??null)?$data['response']:[];
  $challenge=passkey_take_challenge('login');$rawId=passkey_b64_decode((string)($data['rawId']??''));if(strlen($rawId)<1||strlen($rawId)>1024)throw new InvalidArgumentException('Некорректный идентификатор Passkey.');$st=db()->prepare('SELECT p.*,u.is_active,u.auth_version FROM user_passkeys p JOIN users u ON u.id=p.user_id WHERE p.credential_id_hash=? LIMIT 1');$st->execute([hash('sha256',$rawId)]);$key=$st->fetch();$uid=(int)($key['user_id']??0);
  if(!$key||!$key['is_active']||!hash_equals($key['credential_id'],$rawId))throw new InvalidArgumentException('Passkey не найден.');
  $clientData=passkey_client_data((string)($response['clientDataJSON']??''));$server=passkey_server();$server->processGet($clientData,passkey_b64_decode((string)($response['authenticatorData']??'')),passkey_b64_decode((string)($response['signature']??'')),(string)$key['public_key'],$challenge,(int)$key['sign_count'],true,true);$counter=$server->getSignatureCounter()??(int)$key['sign_count'];
  db()->prepare('UPDATE user_passkeys SET sign_count=?,last_used_at=NOW() WHERE id=?')->execute([$counter,$key['id']]);
  session_regenerate_id(true);rotate_csrf_token();$_SESSION['uid']=$uid;$_SESSION['auth_version']=$key['auth_version'];$_SESSION['reauthenticated_at']=time();unset($_SESSION['passkey_setup_required']);db()->prepare('UPDATE users SET last_login_at=NOW() WHERE id=?')->execute([$uid]);json_response(['ok'=>true,'redirect'=>url('/dashboard')]);
 }catch(InvalidArgumentException|\lbuchs\WebAuthn\WebAuthnException $e){json_response(['ok'=>false,'error'=>'Проверка Passkey не пройдена. Повторите попытку.'],422);}
}
if($path==='/api/passkeys/register/options' && $method==='POST'){
 $u=require_recent_auth();$st=db()->prepare('SELECT credential_id FROM user_passkeys WHERE user_id=?');$st->execute([$u['id']]);$server=passkey_server();$args=$server->getCreateArgs(hash('sha256','user:'.$u['id'],true),$u['login'],$u['login'],300,'preferred','required',null,$st->fetchAll(PDO::FETCH_COLUMN));passkey_store_challenge('register',(int)$u['id'],$server->getChallenge());json_response(['ok'=>true,'publicKey'=>$args->publicKey]);
}
if($path==='/api/passkeys/register' && $method==='POST'){
 $u=require_recent_auth();try{
  $response=is_array($data['response']??null)?$data['response']:[];
...
if(preg_match('#^/api/public/todos/([A-Za-z0-9_-]{20,64})/state$#',$path,$m)){
 $st=db()->prepare("SELECT id FROM todo_lists WHERE public_slug=? AND visibility='PUBLIC_READ' AND is_archived=0");$st->execute([$m[1]]);$l=$st->fetch();if(!$l)json_response(['ok'=>false],404);$id=(int)$l['id'];$st=db()->prepare('SELECT id,title,position FROM todo_categories WHERE todo_list_id=? ORDER BY position,id');$st->execute([$id]);$cats=$st->fetchAll();$st=db()->prepare('SELECT id,category_id,title,description,position,updated_at FROM tasks WHERE todo_list_id=? ORDER BY category_id,position,id');$st->execute([$id]);$tasks=$st->fetchAll();$st=db()->prepare('SELECT tt.task_id,g.id,g.name,g.color FROM task_tags tt JOIN tags g ON g.id=tt.tag_id JOIN tasks t ON t.id=tt.task_id WHERE t.todo_list_id=?');$st->execute([$id]);$map=[];foreach($st->fetchAll() as $g)$map[$g['task_id']][]=['id'=>$g['id'],'name'=>$g['name'],'color'=>$g['color']];foreach($tasks as &$t)$t['tags']=$map[$t['id']]??[];json_response(['ok'=>true,'categories'=>$cats,'tasks'=>$tasks]);
}
...
if(preg_match('#^/api/tasks/(\d+)/move$#',$path,$m) && $method==='PATCH'){
 $u=require_role('developer','founder');$tid=(int)$m[1];$st=db()->prepare('SELECT * FROM tasks WHERE id=?');$st->execute([$tid]);$t=$st->fetch();if(!$t||!list_access((int)$t['todo_list_id'],$u,true))json_response(['ok'=>false],404);if($u['role']!=='founder' && (int)$t['created_by']!==(int)$u['id'])json_response(['ok'=>false,'error'=>'Перемещать чужие задачи нельзя'],403);$to=(int)($data['category_id']??0);$c=db()->prepare('SELECT 1 FROM todo_categories WHERE id=? AND todo_list_id=?');$c->execute([$to,$t['todo_list_id']]);if(!$c->fetchColumn())json_response(['ok'=>false,'error'=>'Wrong category'],422);$pos=max(0,min(1000000,(int)($data['position']??0)));db()->prepare('UPDATE tasks SET category_id=?,position=?,version=version+1 WHERE id=?')->execute([$to,$pos,$tid]);audit((int)$t['todo_list_id'],$tid,$u['id'],'TASK_MOVED',(int)$t['category_id'],$to,['position'=>$t['position']],['position'=>$pos]);event_log('TASK_MOVED',"{$u['login']} переместил задачу «{$t['title']}»",$u['id'],(int)$t['todo_list_id'],$tid);json_response(['ok'=>true]);
}
...
if(preg_match('#^/api/todo-lists/(\d+)/settings$#',$path,$m) && $method==='PATCH'){
 ...
 db()->beginTransaction();try{
   db()->prepare('UPDATE todo_lists SET title=?,description=?,visibility=?,public_slug=? WHERE id=?')->execute([$title,$description,$visibility,$slug,$id]);
   foreach($categoryTitles as $cid=>$name){$cid=(int)$cid;$name=trim((string)$name);if($cid>0&&$name!=='')db()->prepare('UPDATE todo_categories SET title=? WHERE id=? AND todo_list_id=?')->execute([$name,$cid,$id]);}
   foreach($tagTitles as $tagId=>$name){$tagId=(int)$tagId;$name=trim((string)$name);if($tagId>0&&$name!=='')db()->prepare('UPDATE tags SET name=? WHERE id=?')->execute([$name,$tagId]);}
...
if(preg_match('#^/api/todo-lists/(\d+)/visibility$#',$path,$m) && $method==='PATCH'){
 $u=require_role('founder');$id=(int)$m[1];$v=$data['visibility']??'TEAM_ONLY';if(!in_array($v,['TEAM_ONLY','SELECTED_USERS','PUBLIC_READ'],true))json_response(['ok'=>false,'error'=>'Некорректный режим видимости'],422);$st=db()->prepare('SELECT visibility,public_slug FROM todo_lists WHERE id=?');$st->execute([$id]);$old=$st->fetch();if(!$old)json_response(['ok'=>false],404);$slug=$v==='PUBLIC_READ'?($old['public_slug']?:bin2hex(random_bytes(16))):null;db()->prepare('UPDATE todo_lists SET visibility=?,public_slug=? WHERE id=?')->execute([$v,$slug,$id]);audit($id,null,$u['id'],$v==='PUBLIC_READ'?'TODO_PUBLIC_ENABLED':'TODO_ACCESS_UPDATED',null,null,$old,['visibility'=>$v]);json_response(['ok'=>true,'public_slug'=>$slug,'public_url'=>$slug?absolute_url('/public/todos/'.$slug):null]);
}
